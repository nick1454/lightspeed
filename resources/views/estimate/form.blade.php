...
<div class="bg-white rounded-2xl shadow border overflow-hidden">
    ...
    <div class="px-6 py-4 border-b flex justify-between items-center">
        ...
        <div class="text-sm text-slate-500">
            Doc # <strong>{{ $item->estimate_no }}</strong>
        </div>
    </div>

    <form action="{{ route('estimate.update', $item->id) }}" method="post">
        <div class="p-6 grid md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <input type="date" id="estimate_date" name="estimate_date" value="{{ $item->estimate_date }}" class="w-full border rounded-xl p-2.5">
            </div>
            <div>
        ...
        <div class="px-6 py-6">
            ...
            <button type="button" onclick="resetForm()" class="bg-slate-200 px-5 py-2.5 rounded-xl">
                Clear
                  ...
                  <a href="{{ route('estimate.list') }}" class="bg-blue-500 text-white px-5 py-2.5 rounded-xl">
                Back
            </a>
        </div>
    </form>
...
<div class="bg-white rounded-2xl shadow border overflow-visible">
    ...
    <div class="p-6">
        <!-- item editor -->
        <form action="{{ route('estimate.items.store') }}" method="post">
            @csrf
            <input type="hidden" id="item_id" name="item_id" value="">
            <input type="hidden" id="material_id" name="material_id">
            <input type="hidden" id="estimate_id" name="estimate_id" value="{{ $item->id }}">
...
<div class="overflow-auto">
    <table class="w-full text-sm">
        ...
        <tbody id="itemTableBody">
            @foreach ($item->estimateItems as $estimateItems)
            <tr class="border-t" id="itemRow_{{ $estimateItems->id }}">
                <td class="p-4" id="material_name_{{ $estimateItems->id }}">
                    <span id="item_id_{{ $estimateItems->id }}" class="hidden">{{ $estimateItems->id }}</span>
                    <span id="material_id_{{ $estimateItems->id }}" class="hidden">{{ $estimateItems->material_id }}</span>
                    {{ $estimateItems->material_name }}
                </td>
                <td class="p-4" id="quantity_{{ $estimateItems->id }}">
                    {{ $estimateItems->quantity }}
                </td>
                <td class="p-4" id="rate_{{ $estimateItems->id }}">
                    {{ $estimateItems->rate }}
                </td>
                <td class="p-4" id="amount_{{ $estimateItems->id }}">
                    {{ $estimateItems->amount }}
                </td>
                <td class="p-4">
                    <button type="button" onclick="editItem({{ $estimateItems->id }})" class="bg-blue-600 text-white px-2 py-1 rounded-full">
                        Edit
                    </button>
                    <button type="button" onclick="deleteItem({{ $estimateItems->id }})" class="bg-red-600 text-white px-2 py-1 rounded-full">
                        Delete
                    </button>
                </td>
            </tr>
...
@section('scripts')
    ...
    function editItem (id) {
        ...
        let material_id1 = document.getElementById(`material_id_${id}`).innerText;
        ...
        material_id.value=material_id1;
