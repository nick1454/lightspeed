@extends('layouts.app')
@section('title', 'Material Outward List')
@section('content')

<main class="p-6 space-y-6">
    <!-- TOP -->
    <div class="flex justify-between items-center">
        <h2 class="text-xl font-semibold">Material Outward List</h2>

        <a href="/materialoutward/form"
    ...
    <div class="bg-white rounded shadow">
        ...
        <table class="w-full text-sm">
            ...
            <thead class="bg-gray-50">
                <tr>
                    <th class="px-4 py-3 text-left">Outward No</th>
            ...
            <tbody>
                ...
                <tr class="border-t">
                    <td class="px-4 py-3">{{ $item->outward_no }}</td>
                    <td class="px-4 py-3">{{ $item->out_date }}</td>
                    ...
                    <td class="px-4 py-3 text-right space-x-2">
                        <a href="{{ route('materialoutward.print', $item->id) }}" title="Print" class="text-green-600" target="_blank">Print</a>
                        <a href="{{ route('materialoutward.edit', $item->id) }}" title="Edit" class="text-blue-600">Edit</a>
                        <button onclick="deleteItem('{{ $item->id }}','{{ route('materialoutward.destroy', $item->id) }}')" class="text-red-600">Delete</button>
                    </td>
                </tr>
                @endforeach
            </tbody>
