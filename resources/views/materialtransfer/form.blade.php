...
<div class="bg-white rounded-2xl shadow border overflow-hidden">
    @if (session()->has('error') || session()->has('success'))
        <div class="p-4 mx-4 mt-4 text-sm @if (session()->has('error')) text-red-800 @else text-green-800 @endif rounded-lg bg-red-50 border border-red-200" role="alert">
    ...
    <div class="px-6 py-4 border-b flex justify-between items-center">
        ...
        <div class="text-sm text-slate-500">
            Doc # <strong>{{ $item->transfer_no }}</strong>
        </div>
    </div>

    <form action="{{ route('materialtransfer.update', $item->id) }}" method="post">
        <div class="p-6 grid md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <input type="date" id="transfer_date" name="transfer_date" value="{{ $item->transfer_date }}" class="w-full border rounded-xl p-2.5">
            </div>
            <div>
                <label class="block mb-1 text-sm font-medium">Warehouse</label>
                <select class="w-full border rounded-xl p-2.5" id="warehouse_from_id" name="warehouse_from_id">
                    <option value="">Select Warehouse</option>
                    @foreach($warehouses as $warehouse)
                    <option value="{{ $warehouse->id }}" {{ $warehouse->id == $item->warehouse_from_id ? 'selected' : '' }}>{{ $warehouse->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block mb-1 text-sm font-medium">Warehouse</label>
                <select class="w-full border rounded-xl p-2.5" id="warehouse_to_id" name="warehouse_to_id">
                    <option value="">Select Warehouse</option>
                    @foreach($warehouses as $warehouse)
                    <option value="{{ $warehouse->id }}" {{ $warehouse->id == $item->warehouse_to_id ? 'selected' : '' }}>{{ $warehouse->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block mb-1 text-sm font-medium">Manual Transfer No</label>
                <input id="manual_transfer_no" name="manual_transfer_no" value="{{ $item->manual_transfer_no }}" class="w-full border rounded-xl p-2.5">
            </div>
        </div>
        <div class="px-6">
            <div>
        ...
        <div class="px-6 py-6">
            <button id="saveTransfer" type="submit" class="bg-blue-600 text-white px-5 py-2.5 rounded-xl">
                Save Transfer
            </button>
            <button type="button" onclick="resetForm()" class="bg-slate-200 px-5 py-2.5 rounded-xl">
                Clear
            </button>
            <a href="{{ route('materialtransfer.list') }}" class="bg-gray-600 text-white px-5 py-2.5 rounded-xl">
                Back
            </a>
        </div>
    </form>
...
<div class="bg-white rounded-2xl shadow border overflow-visible">
    ...
    <div class="p-6">
        <!-- item editor -->
        <form action="{{ route('materialtransfer.items.store') }}" method="post">
            @csrf
            <input type="hidden" id="item_id" name="item_id" value="">
            <input type="hidden" id="material_id" name="material_id" value="{{ $item->id }}">
            <input type="hidden" id="material_transfer_id" name="material_transfer_id" value="{{ $item->id }}">
...
@section('scripts')
    ...
    const materialInId = 1; // current header id
