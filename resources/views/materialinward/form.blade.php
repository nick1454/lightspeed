@extends('layouts.app')
@section('title', 'Material Inward Entry')
@section('content')
...
<div class="bg-white rounded-2xl shadow border overflow-hidden">
    ...
    <div class="px-6 py-4 border-b flex justify-between items-center">
        ...
        <div class="text-sm text-slate-500">
            Doc # <strong>{{ $item->inward_no }}</strong>
        </div>
    </div>

    <form action="{{ route('materialinward.update', $item->id) }}" method="post">
        <div class="p-6 grid md:grid-cols-4 gap-4">
            ...
            <div>
                <label class="block mb-1 text-sm font-medium">Vendor Inward No</label>
                <input id="vendor_inward_no" name="vendor_inward_no" value="{{ $item->vendor_inward_no }}" class="w-full border rounded-xl p-2.5">
            </div>
        </div>
        <div class="px-6">
            <div>
...
<div class="bg-white rounded-2xl shadow border overflow-visible">
    ...
    <div class="p-6">
        ...
        <form action="{{ route('materialinward.items.store') }}" method="post">
            ...
            <input type="hidden" id="material_inward_id" name="material_inward_id" value="{{ $item->id }}">
