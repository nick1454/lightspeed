        ...
        <div class="a4-container">
            ...
            <div class="company-header">
                ...
                <div class="header-right">
                    <img src="https://api.qrserver.com/v1/create-qr-code/?size=70x70&data={{ $jobwork->po_no }}">
                </div>
            </div>

            <div class="title">PO Job Work</div>

            <!-- DETAILS -->
            <table class="header-table">
                <tr>
                    <td>
                        PO No: <strong>{{ $jobwork->po_no }}</strong><br>
                        Date: <strong>{{ date('d-m-Y', strtotime($jobwork->po_date)) }}</strong><br>
                        Vendor: <strong>{{ $jobwork->client->name ?? '' }}</strong>
                    </td>
                    <td>
                        Remarks: <strong>{{ $jobwork->remarks }}</strong><br>
                        Vendor Ref: <strong>{{ $jobwork->vendor_inward_no }}</strong>
                    </td>
                </tr>
            </table>
            ...
            <table class="table">
                ...
                @foreach($jobwork->items as $i => $item)
                @php
                    $gstRate = $item->gst ?? 0;
            ...
            <div class="footer">
                <table width="100%">
                    <tr>
                        <td>
                        Printed: {{ $item->created_at->format('d-m-Y H:i') }}<br>
                        Created By: {{ $item->createdByUser->name ?? '' ?? 'System' }}
                        </td>
